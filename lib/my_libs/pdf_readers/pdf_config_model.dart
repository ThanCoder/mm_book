...
class PdfConfigModel {
  ...
  factory PdfConfigModel.fromCache(String cacheName) {
    try {
      final file = File('${PathUtil.getCachePath()}/$cacheName.json');
      if (file.existsSync()) {
        final json = jsonDecode(file.readAsStringSync());
        return PdfConfigModel.fromMap(json);
      }
    } catch (e) {
      debugPrint('fromCache: ${e.toString()}');
    }
    return PdfConfigModel();
  }
  Future<void> saveConfigCache({required String cacheName}) async {
    try {
      final file = File('${PathUtil.getCachePath()}/$cacheName.json');
      await file.writeAsString(jsonEncode(toMap()));
    } catch (e) {
      debugPrint('setConfigCache: ${e.toString()}');
    }
  }
